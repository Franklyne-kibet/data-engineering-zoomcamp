...
YELLOW_TAXI_CSV_FILE_TEMPLATE = AIRFLOW_HOME + '/yellow_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.gz'
YELLOW_TAXI_PARQUET_FILE_TEMPLATE = AIRFLOW_HOME + '/yellow_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.parquet'
YELLOW_TAXI_GCS_TEMPLATE = "raw/yellow_tripdata/{{ execution_date.strftime(\'%Y\') }}/yellow_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.parquet"
...
download_parquetize_upload_dag(
    ...
    gcs_path_template=YELLOW_TAXI_GCS_TEMPLATE
)

#GREEN TAXI DATA

GREEN_TAXI_URL_TEMPLATE = URL_PREFIX + 'green/green_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.csv.gz'
GREEN_TAXI_CSV_FILE_TEMPLATE = AIRFLOW_HOME + '/green_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.gz'
GREEN_TAXI_PARQUET_FILE_TEMPLATE = AIRFLOW_HOME + '/green_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.parquet'
GREEN_TAXI_GCS_TEMPLATE = "raw/green_tripdata/{{ execution_date.strftime(\'%Y\') }}/green_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.parquet"
...
download_parquetize_upload_dag(
    ...
    gcs_path_template=GREEN_TAXI_GCS_TEMPLATE
)

#FOR HIRE VEHICLES

FHV_TAXI_URL_TEMPLATE = URL_PREFIX + 'fhv/fhv_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.csv.gz'
FHV_TAXI_CSV_FILE_TEMPLATE = AIRFLOW_HOME + '/fhv_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.gz'
FHV_TAXI_PARQUET_FILE_TEMPLATE = AIRFLOW_HOME + '/fhv_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.parquet'
FHV_TAXI_GCS_TEMPLATE = "raw/fhv_tripdata/{{ execution_date.strftime(\'%Y\') }}/fhv_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.parquet"
...
download_parquetize_upload_dag(
    ...
    gcs_path_template=FHV_TAXI_GCS_TEMPLATE
)

#Zones 
ZONES_TAXI_URL_TEMPLATE = 'https://github.com/DataTalksClub/nyc-tlc-data/releases/download/misc/taxi_zone_lookup.csv'
...
ZONES_TAXI_GCS_TEMPLATE = "raw/taxi_zone/taxi_zone_lookup.parquet"
...
download_parquetize_upload_dag(
    ...
    local_csv_path_template= ZONES_TAXI_CSV_FILE_TEMPLATE,
    local_parquet_path_template= ZONES_TAXI_PARQUET_FILE_TEMPLATE,
    gcs_path_template=ZONES_TAXI_GCS_TEMPLATE
)
